    ...
    policy.update(
        {
            ...
            "severity": severity,
            "minimum_build_number": _as_int(data.get("minimum_build_number")),
...
def get_desktop_update_policy(current_build: Optional[int], platform: str = "macos") -> dict[str, Any]:
    doc = db.collection("desktop_update_policy").document("current").get()
    ...
    minimum_build = policy.get("minimum_build_number")
    if current_build is not None and minimum_build is not None and current_build >= minimum_build:
        return _default_policy()
