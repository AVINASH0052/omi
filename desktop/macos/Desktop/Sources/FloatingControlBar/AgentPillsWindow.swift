...
final class AgentPillsWindow: NSPanel, NSWindowDelegate {
  ...
  init() {
    super.init(
      contentRect: NSRect(
        origin: .zero, size: NSSize(width: 200, height: AgentPillsLayout.rowHeight)),
  ...
  private func repositionToAnchor() {
    ...
    let frame = SpatialOverlayGeometry.frameAnchoredBelow(
      ...
      contentSize: NSSize(width: rowWidth, height: totalHeight),
      minimumWidth: AgentPillsLayout.popoverWidth,
      gap: AgentPillsWindow.gapBelowBar
    )
  ...
  private func computeRowWidth(pillCount: Int) -> CGFloat {
    let pillsWidth =
      CGFloat(pillCount) * AgentPillsLayout.pillSize
